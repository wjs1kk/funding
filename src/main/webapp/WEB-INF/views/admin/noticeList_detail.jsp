...
<!-- 2023-05-22 박경은 - 삭제컨펌창 -->
	function deleteNotice(board_idx) {
		if (confirm("정말로 삭제하시겠습니까?")) {
			location.href = 'noticeListDeletePro?board_idx=' + board_idx;
		}
	}
...
<!-- 2023-05-31 박경은 - 삭제컨펌창 -->
	function deleteFile() {
		$.ajax({
			type: "POST",
			url: "BoardDeleteNoticeFile",
			data: {
				"board_idx" : ${notice.board_idx},
				"board_file" : "${notice.board_file}",
				"board_file_path" : "${notice.board_file_path}"
			},
						...
						<div class="row">
							...
							<div class="col-xxl">
								<div class="card mb-4">
									...
									<div class="card-body" id="noticeList_detail">
										<form action="noticeListDetailPro" name="noticeList"
												...
												<div class="col-sm-10">
													<div class="input-group input-group-merge">
														<input type="text" id="board_subject" name="board_subject"
															class="form-control" value="${notice.board_subject}"
															placeholder="제목" aria-label="john.doe"
															aria-describedby="basic-default-email2">
													</div>
												</div>
											</div>
											<!-- 2023-05-25 박경은 - 내용 박스스타일 수정, 파일첨부 추가  -->
											<div class="row mb-3">
												...
												<div class="col-sm-10">
													<textarea placeholder="내용을 입력하세요." class="form-control"
														...
														style="height: 206px;">${notice.board_content}</textarea>
												</div>
											</div>
											<div class="row mb-3">
												...
												<div class="col-sm-10" id="fileBtnArea">
													...
													<c:choose>
														<c:when test="${empty notice.board_file }">
															<input class="form-control" type="file" id="board_file" name="file">
														</c:when>
														<c:otherwise>
															<c:set var="length"
																value="${fn:length(notice.board_file) }" />
															<c:set var="index"
																value="${fn:indexOf(notice.board_file, '_') }" />
															<c:set var="fileName"
																value="${fn:substring(notice.board_file, index + 1, length) }" />

															<div style="padding: 10px">
																<a
																	href="${pageContext.request.contextPath }/resources/upload/${notice.board_file_path}/${notice.board_file}"
											...
											<div class="row justify-content-end">
												<div class="col-sm-10">
													...
													<button type="button" class="btn btn-danger"
														onclick="deleteNotice('${notice.board_idx}')">삭제</button>
												</div>
											</div>
