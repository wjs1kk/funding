				...
				<div class="wpurchase-title">
					<h2>
						${projectInfo.project_title}<span class="name"><em
							style="background-image: url(${pageContext.request.contextPath }/resources/images/maker_images/${projectInfo.maker_image })"></em>농업회사법인
							주식회사 농부건강농원</span>
					</h2>
				</div>
				<div class="wpurchase-wrap " style="">
					<div class="wpurchase-step">
							...
							<div id="reward-product-app" data-campaign-id="215321"
								...
								<ul class="RewardProductList_container__2hIAB">
									...
									<c:forEach items="${selectReward }" var="selectReward" varStatus="status">
										<c:choose>
											<c:when test="${fundingDetail.project_detail_person lt 100}">
												<input type="hidden" id="reward_amount_${status.index}" value="${selectReward.reward_amount-selectReward.reward_amount*0.1  }"></input>
											</c:when>
											<c:when test="${fundingDetail.project_detail_person gt 99 and fundingDetail.project_detail_person lt 500}">
												<input type="hidden" id="reward_amount_${status.index}" value="${selectReward.reward_amount-selectReward.reward_amount*0.2  }"></input>											</c:when>
											<c:when test="${fundingDetail.project_detail_person gt 499 and fundingDetail.project_detail_person lt 1000}">
												<input type="hidden" id="reward_amount_${status.index}" value="${selectReward.reward_amount-selectReward.reward_amount*0.25  }"></input>
											</c:when>
											<c:when test="${fundingDetail.project_detail_person gt 999}">
												<input type="hidden" id="reward_amount_${status.index}" value="${selectReward.reward_amount-selectReward.reward_amount*0.3  }"></input>
											</c:when>
										</c:choose>
											...
											<section
												...
												<label class="RewardProductItem_label__2t2Wd"
													for="checkbox-reward-product-445400"><div
														...
														<c:choose>
															<c:when test="${fundingDetail.project_detail_person lt 100}">
																<input type="checkbox" id="rewardCheckbox_${status.index}"  onclick="rewardDisplay(${selectReward.reward_amount-selectReward.reward_amount*0.1 }, ${status.index}, ${length })">					
															</c:when>
															<c:when test="${fundingDetail.project_detail_person gt 99 and fundingDetail.project_detail_person lt 500}">
																<input type="checkbox" id="rewardCheckbox_${status.index}"  onclick="rewardDisplay(${selectReward.reward_amount-selectReward.reward_amount*0.2 }, ${status.index}, ${length })">											
															</c:when>
															<c:when test="${fundingDetail.project_detail_person gt 499 and fundingDetail.project_detail_person lt 1000}">
																<input type="checkbox" id="rewardCheckbox_${status.index}"  onclick="rewardDisplay(${selectReward.reward_amount-selectReward.reward_amount*0.25 }, ${status.index}, ${length })">
															</c:when>
															<c:when test="${fundingDetail.project_detail_person gt 999}">
																<input type="checkbox" id="rewardCheckbox_${status.index}"  onclick="rewardDisplay(${selectReward.reward_amount-selectReward.reward_amount*0.3 }, ${status.index}, ${length })">
															</c:when>
														</c:choose>
														
														<span><strike>${selectReward.reward_amount }</strike></span>&nbsp;원 
														<span>
														<c:choose>
															<c:when test="${fundingDetail.project_detail_person lt 100}">
																<fmt:parseNumber var="amount" value="${selectReward.reward_amount-selectReward.reward_amount*0.1 }" integerOnly="true"/>
																<fmt:formatNumber value = "${amount }" type="Number" />
															</c:when>
															<c:when test="${fundingDetail.project_detail_person gt 99 and fundingDetail.project_detail_person lt 500}">
																<fmt:parseNumber var="amount" value="${selectReward.reward_amount-selectReward.reward_amount*0.2 }" integerOnly="true"/>
																<fmt:formatNumber value = "${amount }" type="Number" />
															</c:when>
															<c:when test="${fundingDetail.project_detail_person gt 499 and fundingDetail.project_detail_person lt 1000}">
																<fmt:parseNumber var="amount" value="${selectReward.reward_amount-selectReward.reward_amount*0.25 }" integerOnly="true"/>
																<fmt:formatNumber value = "${amount }" type="Number" />
															</c:when>
															<c:when test="${fundingDetail.project_detail_person gt 999}">
																<fmt:parseNumber var="amount" value="${selectReward.reward_amount-selectReward.reward_amount*0.30 }" integerOnly="true"/>
																<fmt:formatNumber value = "${amount }" type="Number" />
															</c:when>
														</c:choose>
														 원
													...
													<li class="RewardProductItem_item__3va6f" ><section
															...
															<div class="RewardProductCard_content__2wx8s">
																<div class="RewardProductCard_quantity__2CSmq">
																	...
																	<c:choose>
																		<c:when test="${fundingDetail.project_detail_person lt 100}">
																			<c:set var="amount" value="${selectReward.reward_amount-selectReward.reward_amount*0.1 }"></c:set>					
																		</c:when>
																		<c:when test="${fundingDetail.project_detail_person gt 99 and fundingDetail.project_detail_person lt 500}">
																			<c:set var="amount" value="${selectReward.reward_amount-selectReward.reward_amount*0.2 }"></c:set>											
																		</c:when>
																		<c:when test="${fundingDetail.project_detail_person gt 499 and fundingDetail.project_detail_person lt 1000}">
																			<c:set var="amount" value="${selectReward.reward_amount-selectReward.reward_amount*0.25 }"></c:set>
																		</c:when>
																		<c:when test="${fundingDetail.project_detail_person gt 999}">
																			<c:set var="amount" value="${selectReward.reward_amount-selectReward.reward_amount*0.3 }"></c:set>
																		</c:when>
																	</c:choose>
																	
																		<button type="button" aria-label="감소 버튼" id="rewardDecreaseBtn_${selectReward.reward_idx}"
																			class="ProductQuantity_minusButton__22tR8" onclick="rewardDecrease(${amount }, ${status.index}, ${length}, ${selectReward.reward_sell }, ${selectReward.reward_quantity })"></button>
																		<input type="number" aria-label="수량 입력" id="rewardQuantity_${status.index}" onchange="rewardOnchange(${amount}, ${status.index}, ${length}, ${selectReward.reward_sell }, ${selectReward.reward_quantity })"
																			data-testid="quantity" value="1">
																		<button type="button" aria-label="증가 버튼" id="rewardIncreaseBtn_${selectReward.reward_idx }"
																			class="ProductQuantity_plusButton__2TJ7g" onclick="rewardIncrease(${amount }, ${status.index}, ${length}, ${selectReward.reward_sell }, ${selectReward.reward_quantity })"></button>
																	</div>
																	<p class="RewardProductCard_price__3t1PE" id="rewardAmount_${status.index}">${amount }</p>
																</div>
															</div>
														</section>
													</li>
						...
						<form action="payment" method="post" onsubmit="return rewardCheck()">
							...
							<div class="btn-wrap">
								<p class="confirm">
									${projectInfo.project_title} <em id="sumTotalNum">0</em>
									원을 참여합니다.
								</p>
						...
						<ul>
							<li>
								...
								<ul>
									...
									<li><a href="http://pf.kakao.com/_pEeEl" target="_blank" rel="noopener noreferrer">공동구매</a></li>
								</ul>
							</li>
							...
							<ul>
									...
									<li><a href="https://www.wadiz.kr/link/fb_reward"
										target="_blank" rel="noopener noreferrer">공동구매</a></li>
								</ul></li>
						</ul>
						<ul>
