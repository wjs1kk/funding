...
<script type="text/javascript">
	...
	// 중복코드를 줄이기 위한 리워드 삭제 함수
	function rwDelete(reward_idx) {
			...
			success: function(jsonRewardList) {
				...
				for(let rewardList of jsonRewardList){
					...
					$("#rewardAddSection").append('<div class="FundingConditionRewardItem_container__1aQKY spacing-4">'
						...
						+'</span>'
						+'</button>'
						+'<button class="Button_button__1e2A2 Button_xs__x1b7M Button_startIcon__19sdm" type="button" id = "deleteBtn" onclick = "rewardDelete('+ rewardList.reward_idx +')">'
						+'<span>'
						+'<svg viewBox="0 0 40 40" focusable="false"'
						+'role="presentation"'
						+'class="withIcon_icon__3lrgp Button_icon__1qsE3"'
		...
		$.ajax({
			 url: 'projectUpdateCheck',
	          ...
	          success: function(response) {
	        	// project_approve_status가 2(승인)가 된 상태에서 project_update_status(수정 권한)가 2(승인)가 되면 리워드 수정 버튼 활성화
